...
const Billing = {
  ...
  _openForm(existingDoc, type) {
    ...
    App.modal(doc ? `Edit ${isSales ? 'Invoice' : 'Bill'}` : `New ${isSales ? 'Sales Invoice' : 'Purchase Bill'}`,
      ...
      <div id="invoice-form">
        ...
        <!-- AI Smart Paste Quick-Fill Banner -->
        <div style="margin-bottom:16px;padding:10px 14px;background:hsl(271,78%,96%);border:1px solid hsl(271,60%,85%);border-radius:var(--radius);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:34px;height:34px;border-radius:8px;background:hsl(271,78%,50%);color:#fff;display:flex;align-items:center;justify-content:center">
              <span class="material-icons" style="font-size:20px">auto_awesome</span>
            </div>
            <div>
              <div style="font-weight:700;font-size:.85rem;color:hsl(271,78%,30%)">✨ AI Smart Fill from WhatsApp / Email <span class="badge" style="font-size:.68rem;padding:2px 6px;background:hsl(271,78%,50%);color:#fff">Commercial 💎</span></div>
              <div style="font-size:.76rem;color:hsl(271,50%,40%)">Paste raw WhatsApp message or email text to auto-fill customer, items, prices, and GST instantly.</div>
            </div>
          </div>
          <button type="button" class="btn btn-sm" onclick="Billing.openAISmartFill('${type}')" style="background:hsl(271,78%,50%);color:#fff;font-weight:600">
            <span class="material-icons" style="font-size:16px">content_paste</span> AI Smart Paste 💎
          </button>
        </div>
  ...
  /* ─── AI Smart Fill from WhatsApp / Email (Commercial Only) ─── */
  openAISmartFill(type) {
    const lic = DB.getLicenseStatus();
    const isSales = type === 'sales';

    if (lic.isTrial) {
      App.modal('💎 Commercial License Feature',
        `
        <div style="text-align:center;padding:12px 0 16px">
          <div style="width:54px;height:54px;border-radius:50%;background:hsl(271,78%,92%);color:hsl(271,78%,50%);display:inline-flex;align-items:center;justify-content:center;margin-bottom:12px">
            <span class="material-icons" style="font-size:28px">auto_awesome</span>
          </div>
          <h3>AI Smart Fill is a Commercial Feature</h3>
          <p style="font-size:.85rem;color:var(--text-secondary);max-width:380px;margin:0 auto 14px">
            Auto-filling invoices directly from raw WhatsApp messages, customer chats, and vendor emails is exclusive to Commercial Licensed users.
          </p>
          <div style="background:var(--bg);padding:12px 16px;border-radius:var(--radius-sm);font-size:.8rem;color:var(--text-secondary);text-align:left;margin-bottom:14px">
            <strong style="color:var(--text-primary)">What this AI feature does for you:</strong>
            <ul style="margin:6px 0 0 18px;padding:0;line-height:1.5">
              <li>Paste any messy WhatsApp order list or email inquiry</li>
              <li>AI automatically extracts client, products, quantities, and rates</li>
              <li>Auto-matches items from your inventory &amp; calculates GST instantly</li>
            </ul>
          </div>
        </div>
        `,
        ...
        <button class="btn btn-primary" onclick="App.openLicenseModal()"><span class="material-icons">vpn_key</span> Activate License Key</button>
        <button class="btn btn-secondary" onclick="App.checkOnlineActivation()"><span class="material-icons">sync</span> Check Online Status</button>
        `,
        'modal-md'
      );
      return;
    ...
    App.modal('✨ AI Smart Fill from WhatsApp / Email',
      ...
      `
      <button class="btn btn-ghost" onclick="App.closeModal()">Cancel</button>
      <button class="btn btn-primary" style="background:hsl(271,78%,50%);border-color:hsl(271,78%,50%)" onclick="Billing.applyAISmartFill('${type}')">
        <span class="material-icons">auto_awesome</span> Parse &amp; Auto-Fill ${isSales ? 'Invoice' : 'Bill'}
      </button>
      `,
      'modal-lg'
    );
  ...
  parseOrderText(text, isSales = true) {
    ...
    for (const rawLine of lines) {
      ...
      // 1. Party detection
      const partyMatch = line.match(/(?:client|customer|supplier|to|from|party|for|bill to|vendor)[:\s]+([^,\n]+)/i);
      if (partyMatch && !matchedParty) {
        const pName = partyMatch[1].trim();
        matchedParty = parties.find(p => p.name.toLowerCase().includes(pName.toLowerCase()) || pName.toLowerCase().includes(p.name.toLowerCase())) || { name: pName, isNew: true };
        continue;
      }

      if (!matchedParty) {
        const directParty = parties.find(p => p.name.toLowerCase() === line.toLowerCase());
        if (directParty) {
          matchedParty = directParty;
          continue;
        }
      }
      ...
      // 2. Extract Discount %
      let discount = 0;
      const discMatch = line.match(/(?:with\s+)?(\d+(?:\.\d+)?)\s*%\s*(?:disc|discount|off)?/i);
      if (discMatch) {
        discount = parseFloat(discMatch[1]) || 0;
        line = line.replace(discMatch[0], ' ');
      }

      // 3. Extract Rate / Price (e.g. @ 1450, Rs. 1450, 1450 each, ₹1450)
      let rate = 0;
      const rateMatch = line.match(/(?:@|rs\.?|inr|₹|price|rate|cost)\s*(\d+(?:\.\d+)?)\s*(?:each|\/pc|\/unit|\/box|\/roll|rs|inr)?/i) ||
                        line.match(/\b(\d{2,7}(?:\.\d+)?)\s*(?:each|\/pc|\/unit|\/box|\/roll|rs|inr)$/i);
      if (rateMatch) {
        rate = parseFloat(rateMatch[1]) || 0;
        line = line.replace(rateMatch[0], ' ');
      }

      // 4. Extract Quantity & Unit
      let qty = 1;
      let unit = 'Nos';
      const qtyMatch = line.match(/^(\d+(?:\.\d+)?)\s*(pcs|nos|box|roll|mtr|pack|set|pair|unit|units|job|kg|gm|visit)\b/i) ||
                       line.match(/\b(\d+(?:\.\d+)?)\s*(pcs|nos|box|roll|mtr|pack|set|pair|unit|units|job|kg|gm|visit)\b/i) ||
                       line.match(/^(\d+(?:\.\d+)?)\s*x\s+/i);
      ...
      if (qtyMatch) {
        ...
        if (qtyMatch[2]) {
          ...
          else if (u.startsWith('job')) unit = 'Job';
          else if (u.startsWith('set')) unit = 'Set';
          else if (u.startsWith('pack')) unit = 'Pack';
          else if (u.startsWith('pair')) unit = 'Pair';
          else if (u.startsWith('mtr')) unit = 'Mtr';
          else if (u.startsWith('visit')) unit = 'Visit';
          else unit = 'Nos';
        }
        line = line.replace(qtyMatch[0], ' ');
      } else {
        const leadNum = line.match(/^(\d+)\s+([a-zA-Z])/);
        if (leadNum) {
          qty = parseInt(leadNum[1]) || 1;
          line = line.substring(leadNum[1].length).trim();
        }
      }

      // Clean leftover punctuation and whitespace
      const desc = line.replace(/^[-\s*•\d+.)@:,]+/, '').replace(/[-\s*•@:,]+$/, '').replace(/\s{2,}/g, ' ').trim();
      if (!desc || desc.length < 2) continue;

      // Match against product catalog
      let matchedProd = catalog.find(p => p.name.toLowerCase() === desc.toLowerCase() || p.sku.toLowerCase() === desc.toLowerCase()) ||
                        catalog.find(p => p.name.toLowerCase().includes(desc.toLowerCase()) || desc.toLowerCase().includes(p.name.toLowerCase()));

      items.push({
        productId: matchedProd ? matchedProd.id : '',
        name: matchedProd ? matchedProd.name : desc,
        hsn: matchedProd ? matchedProd.hsn : (isSales ? '8525' : '8525'),
        qty: qty,
        unit: matchedProd ? matchedProd.unit : unit,
        rate: rate || (matchedProd ? (isSales ? matchedProd.sellingPrice : matchedProd.purchasePrice) : 0),
        discount: discount,
        gstRate: matchedProd ? matchedProd.gstRate : 18
      });
    }

    return { party: matchedParty, items };
  },

  applyAISmartFill(type) {
    const rawText = document.getElementById('ai-raw-text')?.value;
    if (!rawText || !rawText.trim()) {
      App.toast('Please paste your message or load a demo', 'error');
      return;
    }

    const isSales = type === 'sales';
    const parsed = this.parseOrderText(rawText, isSales);

    if (!parsed || !parsed.items.length) {
      App.toast('Could not find line items in text. Try formatting as "Quantity Product @ Price"', 'warning');
      return;
    }
    ...
    if (tbody) {
      tbody.innerHTML = '';
      parsed.items.forEach(item => this._addRow(item));
      this._recalc();
    }
